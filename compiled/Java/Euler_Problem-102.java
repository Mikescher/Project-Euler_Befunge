...
class Program{
private final static String _g = "AR+LCAAAAAAABADVnbuuJMdyRX+lL3StmS6h8p1JEBf6A7myKHnHHYuWPl6xdtShBEjWbksH4nA4V0xWd2VGxmM/7rv9+sdd+GW0X3eNv1r7VXr8QW1fpX2dn1+/jfPj"+
...
private long[]zc(byte[]b){long[]r=new long[80720];for(int i=0;i<80720;i++)r[i]=b[i];return r;}
...
private long gr(long x,long y){return(x>=0&&y>=0&&x<80&&y<1009)?g[(int)(y*80+x)]:0;}
private void gw(long x,long y,long v){if(x>=0&&y>=0&&x<80&&y<1009)g[(int)(y*80+x)]=v;}
...
private int _2(){
    ...
    if(gr(gr(2,3),gr(1,3)+9)!=45)return 5;else return 3;
}
private int _3(){
    gw(4,3,-1);
    return 4;
...
private int _5(){
    if(t0!=32)return 8;else return 6;
}
private int _6(){
    ...
    gw(0,3,(((0>gr(5,4)?1:0)+(0>gr(6,4)?1:0)+(gr(7,4)<=(gr(6,4)+gr(5,4))?1L:0L)!=0)?0:1)+gr(0,3));
    t0=gr(1,3)-999;
    gw(1,3,gr(1,3)+1);

    if((t0)!=0)return 1;else return 7;
}
private int _7(){
...
private int _8(){
    if(t0!=44)return 10;else return 9;
}
private int _9(){
    sa(gr(3,3)*gr(4,3));
    sa(gr(5,3));
